...
### 3.5 `tech.json` 建议新增的中枢延伸/分解/震荡监视字段

根据 [../chanlun/chanlun-rule-spec.md](../chanlun/chanlun-rule-spec.md) 的第 8.12/8.13/8.14 条，后续建议在 `tech.json` 增加一组“可选但结构化”的说明字段，用于表达背驰后的结构去向、同级别分解口径与中枢震荡状态。
...
- 中枢主结论（线段级）与类中枢辅助结论（笔级）分列展示
- 背驰后去向判断（延续/更高级别盘整/更高级别反向）
- 同级别分解是否唯一，或仍处于双解释待确认
- 中枢震荡监视器状态（中线、次级别中枢序列、预警信号）
...
### 4.5 预警触发但未确认信号模板（第29/92课）

当出现 `zs_monitor_alert` 或背驰后去向预警时，文本层必须支持“有预警、无确认”的固定表达模板，避免误导为已触发买卖点。

模板 A（预突破但 3B 未确认）：

```text
结论：结构有向上预警，暂不确认三买。
理由：出现中枢上沿预突破提示，但首次回试已回到中枢区间。
建议：继续观察离开后首次回试质量，未形成“不回中枢”的条件前不升级为确认信号。
```
...
```text
结论：结构有向下预警，暂不确认三卖。
理由：出现中枢下沿预破位提示，但首次回抽已回到中枢区间。
建议：等待后续离开段与首次回抽再次验证，当前仅按风险观察处理。
```
...
```text
结论：高风险观察，反转尚未确认。
理由：背驰后去向偏向更高级别反趋势，但更高级别结构尚未闭合。
建议：在出现完整离开-回抽确认链前，不将其升级为确认反转。
```
...
- 标准中枢字段统一命名为 `zhongshus`。
- 笔级近似字段统一命名为 `lei_zhongshus`。
- 禁止把 `lei_zhongshus` 在文案中简称为“中枢”。
- 文本结论、策略建议、一二三类买卖点默认引用 `zhongshus`。

指标辅助边界（强制）：

- MACD、均线、量能等指标仅用于辅助解释，不得改写 `zhongshus` 主结论。
- 当指标信号与 `zhongshus` 主结论冲突时，文本必须保留“中枢主口径优先”字样，并将指标冲突列入风险提示。
- 不允许仅凭指标金叉/死叉，把“中枢未确认”改写为“买卖点已确认”。
...
- PNG/SVG 必须对中枢与类中枢使用不同图层或不同样式。
- 图例必须出现“中枢（主）”与“类中枢（辅）”两项。
- 若只渲染了类中枢，标题必须显式标注“类中枢视图”，不能写“中枢视图”。
...
## 11. 发布层统一文案片段库（第29/92课相关）

本节定义可直接复用的最小文案片段，目标是保证不同入口在“预警已触发但未确认信号”场景下用词一致。
...
- `结论：出现预警，但当前不构成确认买卖点。`
- `结论：结构偏风险侧，继续观察首次回抽/回试质量。`
- `结论：仅升级风险等级，不升级交易确认等级。`

### 11.3 理由片段

- `理由：监视器触发 {pre_breakout|pre_breakdown}，但首次回抽/回试已回到中枢区间。`
- `理由：背驰后去向暂偏 {higher_level_reverse_trend|higher_level_range}，尚未形成级别闭合。`
- `理由：当前处于 dual_interpretation_pending，结论必须降级为观察态。`

### 11.4 建议片段

- `建议：等待“离开-首次回抽不回中枢”条件成立后再升级。`
- `建议：暂按 watch/pending 管理，不使用 confirmed 标签。`
- `建议：若后续确认失败，回到中枢扩展路径重新评估。`

### 11.5 风险声明片段

- `说明：以上为结构风险提示，不构成投资建议。`
- `说明：中枢主口径优先，指标信号仅作辅助解释。`
- `说明：类中枢结论仅为辅助，不覆盖主结论。`
...
【结构预警（待确认）】
结论：出现预警，但当前不构成确认买卖点。
理由：监视器触发 pre_breakdown，但首次回抽已回到中枢区间。
建议：暂按 watch/pending 管理，等待离开-首次回抽不回中枢条件成立。
说明：中枢主口径优先，指标信号仅作辅助解释，不构成投资建议。
```