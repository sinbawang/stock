...
| D2 | 标准案例包与 review 模板 | 中 | 补一二三类点正反例、区间套、小转大与多级别共振样例 | 进行中 | 已有骨架；当前已补前端可见级别的区间套/小转大卡片（真实 `600900 1m confirmed 3S`、真实 `002555 1m -> 5m` 候选观察链、`5m buy3 -> third_class_confirmed` 对照卡）。`2026-09-12` 新增 [buy-sell-multi-level-example-cases.md](buy-sell-multi-level-example-cases.md)：**标准点/类比点真实样本卡 7 张**（§3，含锚点 / 价格 / 关联中枢 / 依据码 / 退场轨迹）+ **区间套/小转大真实卡片 11 张**（§7 P 组 5 + S 组 2 + A 组 1 + H 组 3；S 组钉 `third_class_confirmed`，A 组钉 `actionable`，H 组钉 `higher_level_confirmed` + 区间套反向确认）+ 两张覆盖矩阵 + 40+ 构造反例索引；并修复消费层 replay 锚点的可复现性缺陷（§5.1）。`2026-09-13` 订正：`higher_level_confirmed` 与 `small_to_large_reverse_confirm` **已观测到**（16 标的 4665 帧中 3 帧，H1–H3）——先前「未观测到」与上两次一样都是语料覆盖不足；剩余主要是高位档样本仍集中在非主配置组合，主配置 `30m→5m` / `day→5m` 上覆盖为空（详见 §7.3）。 |
...
| T3 | 区间套 / 小转大样例回归 | 中 | 给重点 review 样例绑定自动化锚点 | 进行中 | 依赖 BS2-BS5 的主实现逐步稳定。`2026-09-12`：① 标准点与类比点按冻结 fixture 口径落地（[buy-sell-multi-level-example-cases.md](buy-sell-multi-level-example-cases.md) §3）；② 区间套 / 小转大补 **7 张真实卡片**（同页 §7：P 组 5 张 `analysis_cutoffs` 口径 + S 组 2 张杆序号口径）；③ **修复消费层 replay 锚点的可复现性缺陷**（同页 §5.1）——原先 4 个测试模块共 29 个真实 replay 样本依赖未受版本控制的 `build/` 探针与 `data/`，干净机器上收集阶段即报错；现新增 `tests/replay_support.py` + 11 个 replay 窗口 fixture，零宕树实测 230 passed。④ **高位档定性（三次勘误）**：先用 `analysis_cutoffs`（126 帧）得「三档全为 0」，再扩到 5 标的密集网格（2812 帧）得「`third_class_confirmed` 可达、`actionable` 仍为 0」；再扩到本地**全部 16 标的**（4665 帧）后发现 **`actionable` 同样可达**（32 帧），同批语料上 **`higher_level_confirmed` + 区间套反向确认也首次观测到**（3 帧，2026-09-13 补 H 组卡片）。三次「不可达」结论均为**语料覆盖不足**，不是功能缺口。现已有 **11 张区间套真实卡片**（P 组 5 / S 组 2 / A 组 1 / H 组 3）+ 对应非空转守卫；A / H 组需 `tests/fixtures/real/precision/`（重建：`--only precision`）。剩余：高位档样本仍集中在 `5m→1m` / `1m→5m`，主配置 `30m→5m` 上覆盖为空。 |
...
- ~~**RS1 forming 预备态在真实链路上不可达**~~ **已解除（`2026-09-13` 重启）**：forming 改为
  ...
  **线段口径（rev2）**（离开线段 + 反抽线段不进入中枢，与 confirmed 同源；不再接受笔级对），
  ...
  [signal-realtime-lifecycle-design.md](signal-realtime-lifecycle-design.md) §4.1.5。
...
| RS1 | 实时「预备态」（imminent / forming）分层 | 实时 | P1 | 1/2/3 + 类一 / 类二 | 把「背驰已现、待转折确认」升级为 watch 档可操作提示，盘中更早预警且不 repaint | **重启完成（`2026-09-13`）**。原下架结论（`2026-09-12`：判别式空洞 → forming 被 confirmed 全局遮蔽、8 族全 0）保留备查；重启决策改为**纯结构条件**（不加附加确认、允许漂移失效、目标即时提示）。**rev2（同日）**：用户上报 7 例「三卖预备」误报（笔级对/混合口径）→ 三类 `buy_3`/`sell_3` 判据改为与 confirmed **同源线段口径**（离开线段 + 反抽线段不进入中枢；`_find_buy3_tail_segment_pair` / `_find_sell3_tail_segment_pair`），7 例在各自上报时间点回放均不再命中；其余族拆出结构条件 forming 分支。forming 经 `signal_points` 透传（`lifecycle_state=forming`、`active=False`），同族 confirmed 优先，旧 `forming_points` 槽废弃恒空；confirmed 零变化（129 行发射快照逐字节一致）。已知边界：① 一类 / 类一 / 类二 / 二类 forming 0 帧（结构 ⊆ 确认）；② 三类 forming 被 confirmed 同帧遮蔽（线段对 sell 1570/1570、buy 515/515 帧与 confirmed 共存；段层尾部永远落后于笔链 → renewal 恒真）——当前无可展示窗口，保留分支俟 confirmed 收累后成对生效。闸门重写：不变量 + 被遮蔽边界（confirmed>0 且 forming==0）+ 线段判据纯函数用例 + 反空转。详见 [signal-realtime-lifecycle-design.md](signal-realtime-lifecycle-design.md) §4.1.5。 |
...
进展（增量4，2026-09-13——**重启落地，取代下架结论；同日 rev2 修正三类判据**）：

- 决策（用户）：forming 只满足**结构条件**即可，不叠任何附加确认；允许漂移失效；目标是即时提示
  符合理论的买卖点。确认校验增强后置（§4.2.7 item 2 仍暂缓）。
- 实现（rev1→rev2）：三类先以笔级尾部对落地，随后**用户上报 7 例「三卖预备」误报**
  （结构实为「离开笔+反抽笔」或「离开线段+反抽笔」，均非「离开线段+反抽线段」）→ 改为与 confirmed
  **同源线段口径**：`_find_buy3_tail_segment_pair` / `_find_sell3_tail_segment_pair`（离开线段
  low < 下沿 / high > 上沿 + 回试 / 反抽线段守边界，以未确认尾段也计入）；笔级尾部对仅保留给
  笔级中枢回退路径。其余族按「结构条件 ∧ 确认条件」拆出 forming 分支。
- 发射契约变更：forming 由独立 `forming_points` 槽**迁移**到 `signal_points` 载荷
  （`lifecycle_state=forming`、`active=False`、锚点可为未确认笔——§3.3 允许，仅 watch）；
  同帧同族 confirmed 优先（门控名单命中时跳过 forming）；`forming_points` 旧槽**废弃恒空**，
  旧「独立列表契约」红线随之废止（§4.0 历史设想被取代）。
- rev2 实测（`build/probe_sell3_forming_replay.py` + `build/probe_forming_seg_diag.py`）：
  7 例在各自上报时间点回放**全部不再命中**；线段对结构在 4566 帧语料上 sell 1570 / buy 515 帧
  存在但**全部与 confirmed 同帧共存**（confirmed 锚点 = 同一 hold 段末笔，renewal 1570/1570、
  515/515 恒真）→ 三类 forming 被「confirmed 优先」覆盖；结构必因：`segments[-1].end_bi_id !=
  bis[-1].bi_id` 实测 4566/4566 帧（段层尾部永远落后于笔链，尾段后必有 pending 反向笔）。
  三类 forming 当前无可展示窗口（保留分支俟 confirmed 收累后成对生效）；
  buy_1 / buy_1like / buy_2like / sell_2like forming **0 帧**（结构 ⊆ 确认）。
  confirmed 发射快照与重启前**逐字节一致**（129 行）。
- 消费链：发布包 `latest_buy` / `latest_sell` / `latest_overall` / `recent_active` 排除 forming
  （防预备点冒充确认点），预备文案行改由 `signal_points` 提供（旧 `forming_points` 仅回退）；
  小程序 `buyPoints` 页按 `signal_points` 成行、`lifecycle-badge-forming`「预备」渲染即生效（零改动）。
- 回归：闸门 = 载荷不变量 + 被遮蔽边界（confirmed 三类 > 0 且 forming 三类 == 0）+ 线段判据
  纯函数用例（接受合格对 / 拒绝反抽进入中枢 / 拒绝离开未破沿 / 拒绝离开早于中枢起点 / 买卖对称）
  + 反空转（287 帧回放带缓存）；发布包 3 用例（forming 不进最近买卖点 / 同点 confirmed 优先 / 原样透传）。
