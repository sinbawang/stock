...
| RS3 | 收口既有「工程近似」（笔级中枢级别收敛 / 二类首次回抽窗口 / 三类回中枢失效） | 准确性 | P3 | 1/2/3 | 关闭 BS1 差异表遗留近似，降低边界假信号 | 进行中（item2 已落地 / item3 由 RS0 覆盖 / item1 已重定为级别收敛，代码待增量） |
...
- item 1 重定方向（2026-09-11）：原「笔级力度改段级严格口径」被否——笔级中枢冒充操作级别买卖点是
  级别混用，改力度口径不能纠正级别问题。收口改为**级别收敛**：段级中枢成型前操作级别不发一 / 二类点，
  笔级结构仅下沉执行级别 / 区间套。本次仅重定 task 方向，代码实现（`use_segment_divergence == False`
  分支收敛 + 回归）待后续增量。
