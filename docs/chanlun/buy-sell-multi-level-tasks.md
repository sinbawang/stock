...
| T3 | 区间套 / 小转大样例回归 | 中 | 给重点 review 样例绑定自动化锚点 | 进行中 | 依赖 BS2-BS5 的主实现逐步稳定。`2026-09-12`：标准点与类比点已按冻结 fixture 口径落地（[buy-sell-multi-level-example-cases.md](buy-sell-multi-level-example-cases.md) §3）；区间套 / 小转大同样已按该口径补了 **5 张真实卡片**（同页 §7：高级别取 cutoff、次级别按同一时刻 T 截断，覆盖 `standby`/`watch`、3 种 `window_basis_label`），并新增**状态可达性闸门**（126 帧实测：`actionable`、`third_class_confirmed`、`higher_level_confirmed` 均为 **0**，带空转守卫）。待办：① 已查实旧 002555 锚点的可复现性缺陷（同页 §5.1）——`test_build_miniapp_publish_bundle.py` 导入期依赖 gitignore 的 `build/` 探针 + `data/`，在纯净树上**收集阶段即报错**，需迁到冻结 fixture；② 高位档需更大语料才能定性。 |
...
- 验收第 1 条（重点样例可自动化回归支撑）：**部分达成**——标准点 / 类比点的 7 张真实卡片已进
  `tests/test_example_library_real_cases.py`；**区间套 / 小转大的真实卡片（同页 §7，5 张）已按同一闸门口径补上**，
  但旧 `002555`/`600900` 卡片仍未迁到冻结 fixture（同页 §5.1 已实测其可复现性缺陷）。
