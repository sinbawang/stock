...
| D2 | 标准案例包与 review 模板 | 中 | 补一二三类点正反例、区间套、小转大与多级别共振样例 | 进行中 | 已有骨架；当前已补前端可见级别的区间套/小转大卡片（真实 `600900 1m confirmed 3S`、真实 `002555 1m -> 5m` 候选观察链、`5m buy3 -> third_class_confirmed` 对照卡）。`2026-09-12` 新增 [buy-sell-multi-level-example-cases.md](buy-sell-multi-level-example-cases.md)：**真实窗口样本卡 7 张**（含锚点 / 价格 / 关联中枢 / 依据码 / 退场轨迹）+ 覆盖矩阵 + 构造类反例索引（40+ 用例按点类型归类）+ 已知缺口（4 个点类型无真实正例）。剩餘：区间套 / 小转大的卡片仍在 visual-example-library，两张卡片位于末帧未观察到退场。 |
...
| T3 | 区间套 / 小转大样例回归 | 中 | 给重点 review 样例绑定自动化锚点 | 进行中 | 依赖 BS2-BS5 的主实现逐步稳定。`2026-09-12`：标准点与类比点的真实样本按同一口径落地（见 [buy-sell-multi-level-example-cases.md](buy-sell-multi-level-example-cases.md)），区间套 / 小转大可直接复用该口径（`FIXTURES_ROOT` + `analysis_cutoffs` + 固定窗口 replay）补卡；目前其已校验样例（`600900 1m 3S`、`002555 1m -> 5m`）尚未进自动化闸门。 |
...
- 案例包落在 [buy-sell-multi-level-example-cases.md](buy-sell-multi-level-example-cases.md)：7 张真实窗口卡片
  + 覆盖矩阵 + 构造类反例索引（按点类型归类）。
- 验收第 1 条（重点样例可自动化回归支撑）：**部分达成**——标准点 / 类比点的 7 张真实卡片已进
  `tests/test_example_library_real_cases.py`；**区间套 / 小转大的样例仍未进闸门**（见 T3）。
- 验收第 2 条（规则变更能及时暴露）：**达成**——卡片钉住 `signal_bi_id` / `related_zs_id` / `price` /
  `basis` / cutoff 序列五元组，任一变动均指名失败；自证已做。
- 实测缺口：`sell1` / `sell2` / `buy2` / `sell1like` 四个点类型在 21 窗 × 287 帧上**零真实 confirmed**，
  其判定逻辑目前只在构造输入上被验证。原因尚未定性（既可能是门控确实难满足，也可能是判定过严）。
