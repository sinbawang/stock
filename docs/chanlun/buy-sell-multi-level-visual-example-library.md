...
- 它补的是“前端可见级别买侧 confirmed live 对照”缺口；`1m` 侧当前已补到真实 replay 锚点，但仍未拿到稳定的 live 样本。
