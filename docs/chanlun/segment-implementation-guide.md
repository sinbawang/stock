...
- 震荡未破坏时，即使当前同向上笔未创新高，只要紧邻的下一根同向上笔立即创出段内新高，就视为线段仍在延伸、继续推进（对应 `_extend_segment` 的 `reextends_immediately`）；仅对非首段放开（`allow_weak_reextension=bool(segments)`），首段 bootstrap 最左侧上下文不可靠，不做此延伸
...
- 震荡未破坏时，即使当前同向下笔未创新低，只要紧邻的下一根同向下笔立即创出段内新低，就视为线段仍在延伸、继续推进（对应 `_extend_segment` 的 `reextends_immediately`）；仅对非首段放开（`allow_weak_reextension=bool(segments)`），首段 bootstrap 最左侧上下文不可靠，不做此延伸
