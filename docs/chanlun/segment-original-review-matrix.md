...
- 第71课：线段划分标准的再分辨。
- 第78课：继续说线段的划分（第二种情况第二特征序列的包含处理、A+B+C 合一）。
...
| 78 | 第二种情况的第二特征序列分型判断必须严格按包含关系处理；缺口后 C 段未形成第二特征序列分型又直接新高/新低时，A+B+C 只能算一个线段。 | 当前再分辨主路径用 71 课“先破终点/先破起点”捷径（`_rediscriminate_gap_break_detail`），该捷径在判定上蕴含 67 课“第二特征序列分型”（分型“中元素越过左元素”的左条件与捷径“先破终点”几何等价，捷径为急切近似），因此 78 课的包含处理要求被隐含满足；`absorbed_segment_ids` 再吸收路径已与 78 课“A+B+C 合一”规则显式对齐（缺口候选失效 → 旧段延续吸收 B → 同方向 C 经 `_merge_segments_same_direction` 合流）。 | 一致（捷径蕴含分型判据） | 已补 78 课“A+B+C 合一”再吸收边界用例与等价关系锁定：`test_78_a_plus_b_plus_c_merge_marks_reclaim_metadata`、`test_78_second_feature_sequence_left_condition_equals_shortcut`。 |
...
- `books/chanzhongshuochan_lessons/articles_md/lesson_078_教你炒股票78：继续说线段的划分.md`
