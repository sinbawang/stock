...
**候选修法（待决策，A/B 均会改变用户可见发点）：**

| 方案 | 做法 | 效果 | 代价 |
| --- | --- | --- | --- |
| A（推荐） | 门控锚点从「尾部相对位置」改为「**最近一个满足条件的已确认结构**」：类二向前搜索满足 (up-down-up + 力度衰减) 的已确认三连；三类要求 `hold_seg.is_confirmed` 且向前搜索 | 类二不再被尾部滑动撤回；三类不再锚在未确认段 | **发点变化**：`03690 5m` 保留该点、`00700 day` 不再发该点；需全量测影响面 |
| B | 不改发点，仅把「锚点相对位置滑动导致的撤回」记为独立观测（如 `gate_slid`）并加上界断言 | 缺陷仍在，但被量化与监控 | 不修缺陷 |
| C | 仅登记为已知缺陷 | 零风险 | 缺陷留存 |
