...
消失 —— 与 `tests/test_signal_lifecycle_anchor_gate.py` / §3.3 关注的同类缺陷同源
（也与 §3.4 记录的历史 32 条 `vanished_without_break` 可能相关，待复核是否同因）。
...
1. **低风险且红灯明确**：给三类 / 二类的判据补 `is_confirmed` 要求（对齐
   `_has_reverse_turn_after` 的既有做法）。**代价**：已确认三类 / 二类点会收缩。
2. 复核 §3.4 那 32 条 `vanished_without_break` 是否与 4.2.2 同因（若是，则本项从「疑似过宽」
   升级为「已确认缺陷」）。
3. 再决定是否收紧判别式强度（例如要求反向笔突破锚点极值 / 形成反向线段）。
