...
### 4.2 待评估（2026-09-12）：确认判别式可能本身过宽

§4.1.2 的实测反过来指向一个更值得担心的问题：这些判别式在真实窗口上**恒真**，意味着
**「已确认」的实际含义退化为「锚点之后存在一根已确认的同向后续笔」**，而笔严格交替使这几乎
必然成立：
...
即：§2.8 声明的「反向转折已确认」在实现上可能只等价于「后面还有笔」，
与「RS3 已收口既有工程近似」的宣称存在张力。**本页不据此断言一类 / 二类 / 三类点被高估**——
需单独评估：判别式是否应比较**当前结构**（如反向笔是否已突破锚点极值、是否已形成反向线段），
而不是仅比较笔序号。

评估入口：从冻结窗口回放取得的 `buy_2/sell_2/buy_3/sell_3/buy_2like/sell_2like` 确认样本，
逐例人工判定「确认是否名副其实」。
