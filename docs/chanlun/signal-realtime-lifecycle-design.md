...
#### 4.2.14 复核（2026-09-12 第二轮）：翻转点细定位 + 下游遏制方案证伪

**翻转点比上一轮更细**（`build/probe_segment_flip_000651.py`，step=10）：真正的翻转是
cutoff **2610→2620**（`conf_bis` 151→152，即**第 152 根确认笔 bi151 到位**），非上一轮写的 2588→2628。
`_extend_segment(33)` 的原始返回：

| cutoff | end_idx | is_confirmed | break_bi | 结果 |
| --- | --- | --- | --- | --- |
| 2610 | bi35 | 0 | **37** | 段止于 bi35，bi37 有延迟破坏候选 → 19 段 |
| 2620 | bi151 | 0 | **None** | bi37 的破坏候选消失 → 巨型段 → 6 段 |

即**新确认的更深低点 bi151（38.3）作废了 bi37 处的延迟缺口分型破坏**
（`feature_sequence_gap_fractal_delayed_true` 的 look-ahead 语义：缺口分型须后续确认，
后续出现更深同向极值即证伪该破坏）。这是 §4.2.13 结论的更精确机制。

**危害量化（`build/probe_segment_collapse_stale_points.py`）**：塌缩把 cutoff 2600 的
3 个中枢（zs0[41.45] / zs1[39.1,39.33] / zs2[38.69,39.09]）压成 1 个陈旧 zs0[41.45,41.55]；
现价 ~38.3 却只剩两周前的 41.45 中枢，`sell3` 锚在其上。**共 43 个 cutoff（2740~3130+）发出锚在
zs0 的陈旧 `sell3`**；且 cutoff 2860 起巨型段已确认消失（`giant=False`）但 zs0 陈旧态**仍持续**
——损坏比「未确认巨型段」本身更持久，因为一旦 bi33→151 被划成**一根已确认下降段**，
38.3~39.5 的震荡再也建不出中枢（需 3 段重叠），只剩 seg1~4 的古老 zs0。

**下游遏制（distance gate）方案已证伪**（`build/probe_third_class_edge_gap.py`，101 个真实三类发点）：
按「锚点价到关联中枢边沿的价格百分比」根本分不开垃圾与合法——

| 样本 | pct% | 性质 |
| --- | --- | --- |
| `000651-1m sell3@156`（垃圾） | **6.94** | 段塌缩致陈旧锚点 |
| `03690-day sell3@48`（合法迟发） | **39.59** | 大级别趋势远离中枢 |
| `000591-day sell3@52/58`（§3.7 合法卡片） | 14.8 / 18.7 | 合法迟发 |

日线合法三类点离中枢**比垃圾点更远**（趋势走完后中枢自然远在上方/下方）。用价格距离或
`gap/中枢高度` 阈值都会误杀日线合法点却漏掉垃圾（垃圾的中枢高度仅 0.10，比值虚高不可用）。
**结论：此垃圾不可在信号层安全遏制**——下游是忠实地对（错误的）段/中枢结构求值，
