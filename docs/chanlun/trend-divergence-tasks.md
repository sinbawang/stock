...
- `2026-09-14` 修复 03690 首根幽灵价（雪球源侧：09-11 09:35 high 84.629 为假值，同日 1m/5m/30m 均带出；审计见 `build/03690_firstbar_repair_audit.json`）并重跑报告。该假值原把 ZS2 极值抬到 84.629、与 ZS1 `peak_low=84.05` 形成假 GG/DD 回探，`is_zhongshu_expansion` 误判出「(zs1, zs2) 扩张」。修复后：
  - `test_03690_5m_structure_state_keeps_real_completed_then_new_type_chain` 重钉为 `down ongoing(3)`（无 completed 段、consumption=confirmed）；
  - 原 `test_03690_5m_same_level_decomp_down_trend_and_expansion_detected_separately` 改名为 `test_03690_5m_same_level_decomp_down_trend_without_false_expansion` 并重钉为无扩张（其旧断言 `expanded_high == 84.629` 实为幽灵价，非真实锚点）。
- `2026-09-14` 幽灵价治理升级为**常驻闸门**（`src/chanlun/data/hk_phantom_gate.py`）：以腾讯日线为锚（首根 `open←日线 open`；首/尾/盘中越界极值优先用 1m 成分重聚合，否则锚定保守界 `max/min(O,L,C)`）；仅严重度 ≥0.5% 自动修复，轻微越界/漂移只记 soft；幂等。`batch_prepare_chanlun_reports.py` 对 HK `1m/5m/30m` 在合并后自动过闸并把修复回写本地 store（首尾兜底防增量回抓复染）。历史回填脚本 `scripts/hk_phantom_gate_backfill.py`（`--apply`），审计 `build/hk_phantom_gate_audit.json`：两仓库 10 标的共修复 **1874** 处字段（首根陈旧开盘价 870 处、幽灵价带跑 L/H 234 处、越界极值 770 处），soft 632 处仅记录；回填后两仓库残余 ≥0.5% 越界为 **0**（`build/_check_residual_severe.py` 复核），复跑回填产生 0 修复（幂等）。冻结夹具（`tests/fixtures/real/replay/`）未改动，个别旧夹具仍保留幽灵价快照。
...
- 当前实际修正任务已收口：主扫描脚本默认已切到细粒度 `step=1`，最新 `scan_real_candidate_new_type_samples_latest.json` 也已直接产出多条 `exact_candidate_matches`。这说明先前的 coarse-scan 假阴性问题已经解除，后续重点转为把这些 exact matches 绑定到 review / consumer 样例，而不是继续解释粒度冲突。
