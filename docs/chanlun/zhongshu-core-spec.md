...
### 10.1 “已确认 + 固定区间” vs “3 段重叠即成立”的取舍（已冻结）

两种口径对比结论已定，不再二选一：

- **主链口径（当前实现，实然）**：只吃已确认线段 + 固定前三段重叠区间 + 输入至少 5 段。
  - 优点：重复重算零漂移，`confirmed / pending / auxiliary` 三态清晰，买卖点绑定最近中枢不漂。
  - 代价：滞后于理论最小中枢；但 2026-08-30 起循环上界已放宽为 `i < len(items) - 3`，链尾「进入段 + 3 本体段」的 4 段最小中枢会立即以未终结（`exit=None`）成立，不再等待离开候选。
- **严格理论口径（未落地）**：3 段重叠即形成最小中枢，区间可随段推进动态更新。
  - 优点：理论正确、识别及时。
  - 代价：第三段未封口时区间会漂移，需额外维护“临时 / 定型”状态，不适合直接驱动主结论。

工程决策：

- 主消费链继续采用主链口径，不因“快一段”牺牲稳定性。
- 严格理论口径保留为后续 theory 视图或 pending → confirmed 状态升级方向，当前不进入主产物。
- 输入资格细节见 [zhongshu-input-qualification.md](zhongshu-input-qualification.md)。
