...
>
> 另注：当前实现要求输入至少 5 段（`if len(items) < 5` 直接返回空）；但循环上界
> 已放宽为 `i < len(items) - 3`，因此“进入段 + 3 本体段”落在链尾的 4 段最小中枢会
> 立即以**未终结**成立（`exit_bi_id=None`），不再等待其后的离开候选出现；离开候选
> 仅用于后续的延伸/终结判定。未确认尾段仍被 `_primary_segment_items` 裁掉、不参与
> 区间固定，故区间不会漂移。
