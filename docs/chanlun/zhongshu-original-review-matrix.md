...
如需先看本轮 review 入口而不是直接进入课次矩阵，可先读 [zhongshu-review-entry.md](zhongshu-review-entry.md)。
...
7. [x] 产出第二批样例包（每市场至少3例，含跨市场补充案例）。
8. [x] 产出第二批阈值回放记录（含分市场/分级别统计）。
9. [x] 产出第二批文案抽检记录（50条样本，含入口级差异统计）。
