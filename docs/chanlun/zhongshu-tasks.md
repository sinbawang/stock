...
| T3 | reclaim / gap / rewrite focused regressions | 中 | 继续锁复杂交界，不让旧中枢幽灵回归 | 进行中 | bootstrap / gap / reclaim / reverse_break 已有多组真实 fixture regression；`2026-08-23` 又补了首选级别（1m/5m）多中枢真实窗口 gate（`600900 1m`、`09988 1m`、`03690 5m`、`00700 1m`），并确认真实 `reabsorbed lineage` 样本为确定性数据缺口（全量 558 窗口 + 09988 1334 瞬态均 MATCHED 0），复杂交界缺口已进一步收窄。`2026-09-12` 新增下游链完整性闸门 `tests/test_downstream_chain_integrity.py`（已注册进 `scripts/run_segment_safety_gates.py` 的 `regression` 闸门）：对全部冻结真实窗口断言「段 → 标准中枢」与「确认笔 → 笔级类中枢」不出现静默停滞（先以探针测量、确认无退化后再固化，避免闸门制造噪声）。 |
...
| `1m confirmed 3S` 确认链闭合 | `zhongshu-review-entry.md` 第 92 课 | `zhongshu-visual-example-library.md` `4.5`；`zhongshu-consumer-display-examples.md` 第 7 节 | `tests/test_build_miniapp_publish_bundle.py::test_build_summary_and_detail_payload_preserve_real_00700_5m_confirmed_sell3_sample`；`1m-confirmed-3s-reference-gate` | 作为 `1m pre_break*` 的 confirmed 对照锚点，不与 pending/watch 混写；真实 live 主锚点已从 `600900 1m` 迁移到 `00700 5m sell3/confirmed`（`600900 1m` 结构刷新后不再产出确认三类点），reference gate 继续兜底 | 真实 live 样本与 reference gate 均已落地 |
...
| `1m candidate_new_type` 候选态 | `zhongshu-review-entry.md` 第 39/92 课衔接说明位 | `zhongshu-visual-example-library.md` `1.3.1`；`zhongshu-consumer-display-examples.md` `7.5.1` | `build/scan_real_candidate_new_type_samples_latest.json` exact match（`00175 1m @ 2026-08-26 10:51`） | 约束“前段已完成 + 当前仅 1 个新中枢”只能按 `candidate_new_type + pending` 消费，不得偷换成 `completed_then_new_type` 或 confirmed | 已有真实 exact-match 锚点，后续可继续补具名 pytest / publish gate |
...
| confirmed live gates 已落地 | `tests/test_build_miniapp_publish_bundle.py::test_build_summary_and_detail_payload_preserve_1m_confirmed_3s_reference_anchor` + `tests/test_build_miniapp_publish_bundle.py::test_build_summary_and_detail_payload_preserve_real_01024_1m_down_warning_sample` + `tests/test_build_miniapp_publish_bundle.py::test_build_summary_and_detail_payload_preserve_real_002555_1m_sell3_sample` + `tests/test_build_miniapp_publish_bundle.py::test_build_summary_and_detail_payload_preserve_real_600900_1m_buy3_pending_sample` | 兜底 reference 仍锁 confirmed 3S；真实 `01024/002555/600900` 样本因数据刷新后结构变动，bundle 层改为锁当前态：`01024` 向下预警 + 已确认消费但无确认买卖点，`002555` 三卖 + 待确认消费，`600900` 三买雏形 + 待确认消费。 |
