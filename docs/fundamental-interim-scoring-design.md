...
这份文档主要解释 blended 评分为什么这样设计，并记录当前已落地实现背后的边界选择；它不重复展开所有代码细节。
...
截至 2026-05-31，当前基本面单期 live 评分的默认行为可以总结成一句话：
...
如果只走单期评分链路，一旦出现下面这种情况：

- 2025-12-31 年报已经披露
- 2026-03-31 一季报也已经披露

当前单期评分仍会继续锚定 2025 年报。

这个行为的优点是口径稳定，缺点是：

- 评分新鲜度可能落后于公司最新经营变化

也正因为这个缺点，当前仓库已经补上 blended 链路：

- 数据层会并行抓 `annual + latest_interim`
- 服务层会构造 `AnnualAnchorScore`、`InterimOverlayScore` 和 `BlendedFundamentalScoreCard`
- 报告层已经能输出 blended 简报和 blended 纯文本评分卡

因此，这份文档当前更适合作为“为什么 blended 要这样实现”的设计说明，而不是“是否要做”的前置提案。
...
当前仍停留在离散档位，没有上连续时间衰减模型。

## 8. 报告层展示

当前报告层不会只展示一个最终分。

当前 blended 报告至少会围绕以下信息展开：
...
1. CN / HK 两条链路都已支持 `annual + latest_interim` 双层对象
2. blended 报告与落盘 helper 已经接上
3. CLI 已能切换单期与 blended 输出
4. HK 侧仍保留按子模型渐进开放的边界

不过当前仍保留几个明显边界：

- 还没有连续时间衰减
- 还没有把所有 CLI 细粒度参数都开放出来
- 还没有把所有 HK 子模型无条件放开到 blended 路径
...
1. 扩大 blended 子模型覆盖
2. 把 `weighting_profile`、`annual_period`、`interim_period` 等更细参数逐步开放到 CLI
3. 补齐批量脚本与导出链路
4. 再考虑更复杂的权重或 freshness 策略

## 13. 当前需要明确的开放问题

真正还值得继续拍板的，不多，但仍需要明确：
...
4. 后续是否继续扩大 HK blended 覆盖范围，还是继续按白名单逐步放开
5. 是否要把更多 blended 参数直接暴露给 CLI 与批量脚本
...
- 当前已同时支持 CN / HK，但 HK 继续按白名单渐进开放更稳妥
- 报告里继续直接展示锚定分、刷新分和加权结果，避免黑盒
...
这条结论当前已经进入实现。正确方向也仍然不是“再做一套完整季报评分系统”，而是：
