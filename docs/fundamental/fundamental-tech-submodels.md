...
当前这些科技子模型已经落成 Python 配置对象；对象结构说明见 [fundamental-submodel-config.md](fundamental-submodel-config.md)。
...
- 吉利仍保留为“科技相邻行业”的候选，不纳入当前已落地科技子模型
- 中航科工已经先复用 `industrial_automation_v1`

这样做的含义不是把中航科工定义成和汇川完全同类，而是先按“订单、应收、存货、现金流验证”这一套高端装备制造口径观察。
...
以下表格分成“当前已落地”和“仍保留为边界候选”两部分，避免再把已实现配置和早期规划混在一起。
...
| 中航科工 | 工业自动化与智能装备扩展观察组 | 当前复用 `industrial_automation_v1` |
| 中国电信港股 | 数字基础设施/通信运营 | 当前已拆到独立 `digital_infra_v1`，不再放入科技主模型 |
| 太阳能 | 公用事业与新能源运营 | 当前已拆到独立 `utility_operator_v1` |

仍保留为边界候选的标的：

| 标的 | 候选归类 | 当前处理建议 |
| --- | --- | --- |
| 吉利 | 科技相邻行业：智能电动化制造 | 继续作为边界样本观察，暂不并入当前科技主模型 |
...
所以当前实现已经把它从“科技主模型边界样本”推进到独立行业桶：
...
- 复用 `industrial_automation_v1`

如果后续军工装备字段和工业自动化字段差异继续放大，再单独拆分。

## 9. 第一阶段实现优先级

如果只看科技子模型内部，第一阶段落地顺序仍然可以按这个理解：
...
- 科技行业必须再细分
- 平台互联网、半导体、工业自动化、游戏内容都已经落成配置对象
- 中航科工当前先复用工业自动化与智能装备子模型
- 中国电信和太阳能已经不再只是“不要放进科技桶”的建议，而是分别落入独立行业桶
- 吉利仍然保留在科技相邻行业候选区，暂不硬塞进纯科技桶