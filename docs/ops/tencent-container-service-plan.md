...
当前容器内半小时档位默认调度命令会显式生成 `30m/5m/1m`，并发布 `30m/5m/1m/day` 图表；如果把 `--tech-timeframes` 缩到只剩 `5m`，则不会产出新的 `30m` 图表。
...
- 默认半小时档位命令会请求 `30m/5m/1m` 技术生成，并把 `30m/5m/1m/day` 纳入发布包，保证首页时间戳与 30M 图表同步前进。
