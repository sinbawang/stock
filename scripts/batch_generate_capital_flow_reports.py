...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Batch generate A-share capital-flow scorecard reports.")
    parser.add_argument(
        "--holdings-file",
        default=str(DEFAULT_HOLDINGS_FILE),
        help="Holdings JSON file. Defaults to data/_meta/current_a_share_holdings.json.",
    )
    ...
    parser.add_argument("--no-fallback", action="store_true", help="Disable low-confidence THS fallback when Eastmoney and cache are unavailable")
...
def _infer_market(symbol: str, explicit_market: str | None = None) -> str:
    if explicit_market:
        return explicit_market.upper()
    return "HK" if len(symbol.strip()) == 5 else "CN"
...
def discover_targets_from_holdings_file(holdings_file: Path) -> list[CapitalFlowTarget]:
    ...
    if isinstance(markets, dict):
        ...
        market_code = payload.get("market")
        raw_entries.extend((market_code, entry) for entry in payload.get("holdings", []) if isinstance(entry, dict))

    dedup: dict[str, CapitalFlowTarget] = {}
    for market_code, entry in raw_entries:
        symbol = str(entry.get("symbol") or "").strip()
        name = str(entry.get("name") or "").strip()
        market = _infer_market(symbol, str(market_code or "").strip() or None)
        if not symbol or not name or market != "CN":
            continue
        normalized_symbol = _normalize_cn_symbol(symbol)
        dedup[f"{normalized_symbol}:{name}"] = CapitalFlowTarget(
...
def generate_one(
    ...
    result = fetch_and_analyze_cn_flow(
        target.symbol,
        target.name,
    ...
    report_path = save_capital_flow_text(
...
def _build_overall_comment(results: list[BatchCapitalFlowResult]) -> str:
    ...
    if not succeeded:
        return "本次未生成有效资金面评分，需先修复数据源或缓存。"
    ...
    fallback_count = sum(1 for item in succeeded if _source_bucket(item.source) == "fallback")
    cache_count = sum(1 for item in succeeded if _source_bucket(item.source) == "cache")

    if average_score >= 75 and weak_count == 0:
        tone = "整体资金面偏强"
    elif weak_count >= max(1, len(succeeded) // 2):
        tone = "整体资金面偏弱"
    else:
        tone = "整体资金面分化"
    ...
    if fallback_count:
        quality_parts.append(f"{fallback_count} 只使用 fallback 低置信度口径")
    if cache_count:
        quality_parts.append(f"{cache_count} 只使用缓存口径")
    if failed_count:
        quality_parts.append(f"{failed_count} 只失败")
    quality_text = "；" + "，".join(quality_parts) if quality_parts else ""
    return f"{tone}，有效样本均分 {average_score:.1f}{quality_text}。"


def _sorted_results(results: list[BatchCapitalFlowResult]) -> list[BatchCapitalFlowResult]:
    return sorted(
        results,
        key=lambda item: (item.status != "ok", -(item.total_score or -1), item.target.symbol),
    )
...
def run_batch(
    ...
    for target in targets:
        try:
            ...
            results.append(BatchCapitalFlowResult(target=target, status="failed", error=str(exc)))
            if fail_fast:
                break
    return results
...
def main() -> None:
    ...
    if not targets:
        raise RuntimeError(f"No valid CN holdings found in: {holdings_file}")
