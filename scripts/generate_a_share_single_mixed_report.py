...
def _save_technical_report(
    *,
    ...
    rows = fetch_kline(symbol, start=start, end=end, interval="60m", adjust=adjust, limit=5000)
    if not rows:
    ...
    output_path = output_dir / f"{normalized_symbol}_{name}_tech_60m_{generated_at.strftime('%Y%m%d_%H%M%S')}.txt"
    report_text = "\n".join(
        [
