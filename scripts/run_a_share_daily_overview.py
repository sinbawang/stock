...
import batch_generate_capital_flow_reports as capital_batch
import generate_a_share_combined_overview as combined_overview
...
class AShareDailyOverviewResult:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Generate A-share capital-flow reports and the combined management overview in one run.")
    ...
    parser.add_argument("--no-fallback", action="store_true", help="Disable low-confidence fallback sources")
...
def save_daily_overview_manifest(
    *,
    ...
    manifest_path = meta_dir / f"a_share_daily_overview_manifest_{generated_at.strftime('%Y%m%d_%H%M%S')}.json"
    payload = {
        "task": "a_share_daily_overview",
...
def run_daily_overview(
    ...
    if not combined_targets:
        raise RuntimeError(f"No valid A-share holdings found in: {holdings_file}")
    ...
    return AShareDailyOverviewResult(
