...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="一键执行 A 股 60M 缠论出图、分析")
...
def analyze_current_state(
    ...
    if current_zs:
        focus_lines.append(
            f"观察重点：是否围绕中枢 {current_zs.zs_low:.2f}-{current_zs.zs_high:.2f} 震荡后选择方向，以及后续一笔是否形成有效突破"
        )
...
def main() -> None:
    ...
    normalized_symbol = args.symbol.lower()
    if normalized_symbol.startswith(("sz", "sh", "bj")):
        normalized_symbol = normalized_symbol[2:]

    paths = build_paths(normalized_symbol, args.name, rows)
