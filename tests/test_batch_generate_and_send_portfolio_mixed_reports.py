...
def test_generate_bundle_runs_four_timeframe_chart_pipeline(monkeypatch, tmp_path):
    ...
    assert commands[1][commands[1].index("--zhongshu-level") + 1] == "segment"
    ...
    assert commands[1][commands[1].index("--m5-bars") + 1] == "2000"
    assert commands[1][commands[1].index("--m1-bars") + 1] == "3500"
